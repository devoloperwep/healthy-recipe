...
function ProductsItem({ data, selected, cookSelected, search }) {
  ...
  if (selected) {
    filtered = filtered.filter((product) => product.prepMinutes <= selected);
  }

  if (cookSelected) {
    filtered = filtered.filter(
      (product) => product.cookMinutes <= cookSelected
    );
  }
      ...
      {filtered.length > 0 ? (
        filtered.map((product) => (
          <li key={product.id} className="product__item">
            <picture>
              <source media="(max-width: 500px)" srcSet={product.image.small} />
              <img
